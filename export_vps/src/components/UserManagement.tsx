...
// Hilfsfunktion fÃ¼r eindeutige Mitarbeiternummer (zufÃ¤llig zwischen 0001-9999)
const generateUniqueMitarbeiterID = (existingEmployees: Employee[]): number => {
  ...
  let attempts = 0;
  const maxAttempts = 100; // Maximale Versuche, um eine eindeutige Nummer zu finden
  
  while (attempts < maxAttempts) {
    // ZufÃ¤llige Nummer zwischen 1 und 9999 generieren
    const randomID = Math.floor(Math.random() * 9999) + 1;
    
    // PrÃ¼fen, ob die Nummer bereits verwendet wird
  ...
  // Fallback: Erste verfÃ¼gbare Nummer finden
  ...
  // Wenn alle Nummern verwendet sind, zufÃ¤llige zurÃ¼ckgeben
...
const UserManagement: React.FC<UserManagementProps> = ({ onBack, onNavigate, onOpenMessaging }) => {
  ...
  const {
    ...
    validationRules: {
      firstName: [
        validationRules.required('Der Vorname ist erforderlich'),
        validationRules.minLength(2, 'Der Vorname muss mindestens 2 Zeichen lang sein'),
        validationRules.maxLength(50, 'Der Vorname darf maximal 50 Zeichen lang sein'),
      ],
      lastName: [
        validationRules.required('Der Nachname ist erforderlich'),
        validationRules.minLength(2, 'Der Nachname muss mindestens 2 Zeichen lang sein'),
        validationRules.maxLength(50, 'Der Nachname darf maximal 50 Zeichen lang sein'),
      ],
      email: [
      ...
      role: [
        validationRules.required('Bitte wählen Sie eine Rolle aus'),
      ],
    },
          ...
          if (concernUsers.length > 0) {
            ...
            const firestoreEmployees: Employee[] = concernUsers.map((fsUser: any) => {
              ...
              return {
                ...
                lastLogin: fsUser.lastLogin ? new Date(fsUser.lastLogin) : undefined,
  ...
  const syncWithFirestore = async () => {
    ...
    try {
      ...
      // First, initialize lastLogin for existing users if needed
      console.log('🔄 Initialisiere lastLogin Feld für bestehende Benutzer...');
      const firestoreUsers = await userService.getAll(user.concernID);
      const concernUsers = firestoreUsers.filter(fsUser => !fsUser.isDemoUser);
      
      let lastLoginUpdatedCount = 0;
      
      // Update lastLogin for users who don't have it
      for (const fsUser of concernUsers) {
        if (!fsUser.lastLogin && fsUser.uid) {
          try {
            await userService.update(fsUser.uid, {
              lastLogin: fsUser.dateCreated || new Date()
            });
            lastLoginUpdatedCount++;
            console.log(`✅ lastLogin für ${fsUser.email} initialisiert`);
          } catch (error) {
            console.error(`❌ Fehler beim Aktualisieren von ${fsUser.email}:`, error);
          }
        }
      }
      
      if (lastLoginUpdatedCount > 0) {
        console.log(`✅ ${lastLoginUpdatedCount} Benutzer aktualisiert`);
        toast({
          title: 'Datenbank aktualisiert',
          description: `${lastLoginUpdatedCount} Benutzer haben jetzt ein lastLogin Feld.`,
        });
        
        // Reload data after updating lastLogin
        const updatedFirestoreUsers = await userService.getAll(user.concernID);
        const updatedConcernUsers = updatedFirestoreUsers.filter(fsUser => !fsUser.isDemoUser);
        concernUsers.length = 0;
        concernUsers.push(...updatedConcernUsers);
      }
  ...
  const sortedEmployees = [...filteredEmployees].sort((a, b) => {
    ...
    // Sortierung für Datumsfelder (Date-Objekte)
    if (aValue instanceof Date && bValue instanceof Date) {
      return sortDirection === 'asc' ? aValue.getTime() - bValue.getTime() : bValue.getTime() - aValue.getTime();
    }
    
    // Fallback für undefined/null Werte
      ...
      <div className="min-h-screen tradetrackr-gradient-blue">
        ...
        <div className="p-6">
          <div className="max-w-5xl mx-auto">
            <Card className="border-4 border-[#058bc0] shadow-2xl bg-white overflow-hidden">
              <CardHeader className="bg-gradient-to-r from-[#058bc0] to-[#0470a0] text-white px-6 py-5 -m-6 mb-0">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 max-h-[70vh] overflow-y-auto px-2">
                  ...
                  <div className="space-y-4 p-4 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg border-2 border-blue-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormInput
                        label="Vorname"
                        tooltip="Geben Sie den Vornamen des Mitarbeiters ein. Der Vorname muss zwischen 2 und 50 Zeichen lang sein."
                        ...
                        onChange={(e) => {
                          ...
                          getFieldProps('firstName').onChange(value);
                        }}
                      ...
                      <FormInput
                        label="Nachname"
                        tooltip="Geben Sie den Nachnamen des Mitarbeiters ein. Der Nachname muss zwischen 2 und 50 Zeichen lang sein."
                        ...
                        onChange={(e) => {
                          ...
                          getFieldProps('lastName').onChange(value);
                        }}
                  ...
                  <div className="space-y-4 p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border-2 border-green-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormInput
                        label="E-Mail"
                        type="email"
                        tooltip="Geben Sie die E-Mail-Adresse des Mitarbeiters ein. Diese wird für die Anmeldung und Benachrichtigungen verwendet. Beispiel: max.mustermann@example.com"
                        ...
                        {...getFieldProps('email')}
                        helperText="E-Mail-Adresse für Anmeldung und Benachrichtigungen"
                      />
                      <FormInput
                        label="Telefon"
                        type="tel"
                        tooltip="Geben Sie die Telefonnummer des Mitarbeiters ein (optional). Format: +49 123 456789 oder 0123 456789"
                        ...
                        {...getFieldProps('phone')}
                        helperText="Telefonnummer für Kontaktaufnahme (optional)"
                      />
                    </div>
                  </div>
                  
                  {/* Section 3: Persönliche Details */}
                  <div className="space-y-4 p-4 bg-gradient-to-br from-yellow-50 to-orange-50 rounded-lg border-2 border-yellow-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <Label htmlFor="birthday" className="text-sm font-semibold text-gray-800 flex items-center gap-1">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="position" className="text-sm font-semibold text-gray-800 flex items-center gap-1">
                          <span className="text-base">🔢</span>
                          Alter
                        </Label>
                        <Input 
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="startDate" className="text-sm font-semibold text-gray-800 flex items-center gap-1">
                          <span className="text-base">📅</span>
                          Startdatum in der Firma
                        </Label>
                        <Input 
                          id="startDate" 
                          type="date" 
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="address" className="text-sm font-semibold text-gray-800 flex items-center gap-1">
                          <span className="text-base">📍</span>
                          Adresse
                        </Label>
                        <Input 
                  ...
                  {/* Section 4: Rolle & Berechtigungen */}
                  <div className="space-y-4 p-4 bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg border-2 border-purple-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormSelect
                        label="Rolle"
                        tooltip="Wählen Sie die Rolle des Mitarbeiters aus. Die Rolle bestimmt die Berechtigungen und den Zugriff auf verschiedene Funktionen. Admin hat volle Rechte, Office hat erweiterte Rechte, Project Manager verwaltet Projekte, Service Technician ist für die Ausführung zuständig, Auftraggeber hat eingeschränkten Zugriff."
                        options={[
                          { value: 'admin', label: '👑 Administrator' },
                          { value: 'office', label: '📋 Büro-Mitarbeiter' },
                          { value: 'project_manager', label: '🔧 Projektleiter' },
                          { value: 'service_technician', label: '🚗 Außendienst-Mitarbeiter' },
                          { value: 'auftraggeber', label: '👤 Auftraggeber' },
                        ]}
                        value={formData.role || 'service_technician'}
                        onValueChange={(value) => {
                          setFormData(prev => ({ ...prev, role: value as UserRole }));
                          getFieldProps('role').onChange(value);
                        }}
                        {...getFieldProps('role')}
                        placeholder="Rolle auswählen"
                        helperText="Die Rolle bestimmt die Berechtigungen"
                      />
                      <div className="space-y-2">
                        <Label htmlFor="mitarbeiterID" className="text-sm font-semibold text-gray-800 flex items-center gap-1">
                          <span className="text-base">🔢</span>
                          Mitarbeiter-Nr.
                        </Label>
                        <Input 
                  ...
                  <div className="space-y-4 p-4 bg-gradient-to-br from-gray-50 to-slate-50 rounded-lg border-2 border-gray-200">
                    ...
                    <div className="flex items-center space-x-3 p-3 bg-white rounded-lg border-2 border-gray-200">
                      <Switch 
                        checked={formData.isActive || false} 
                        onCheckedChange={(checked) => setFormData(prev => ({ ...prev, isActive: checked }))} 
                      />
                      <Label className="text-sm font-semibold text-gray-800 flex items-center gap-1">
                        <span className="text-base">{formData.isActive ? '✅' : '⏸️'}</span>
                  ...
                  {editingUser && (
                    <div className="space-y-4 p-4 bg-gradient-to-br from-cyan-50 to-blue-50 rounded-lg border-2 border-cyan-200">
                  ...
                  <div className="flex justify-end gap-3 pt-6 border-t-2 border-gray-300">
                    ...
                    <Button 
                      ...
                      className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-semibold shadow-lg hover:shadow-xl transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {syncStatus === 'syncing' ? '🔄 Synchronisiere...' : 
             ...
             syncStatus === 'conflict' ? '⚠️ Konflikte' : '🔄 Sync'}
          </Button>
                          ...
                          <TableHeader className="sticky top-0 bg-gray-50 z-10">
                            ...
                            <TableHead className="w-1/4">
                              <Button
                                ...
                                className="h-8 flex items-center gap-1 hover:bg-gray-100 w-full justify-start"
                              >
                                Name
                            ...
                            <TableHead className="w-1/12 text-center">Aktionen</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {sortedEmployees.map((employee) => (
                            ...
                            >
                              ...
                              <TableCell className="align-top">{getRoleBadge(employee.role)}</TableCell>
                                ...
                                {employee.lastLogin ? (
                                  <div className="text-sm text-gray-600">
                                    ...
                                    <span className="text-xs text-gray-500">
                                      {new Date(employee.lastLogin).toLocaleTimeString('de-DE')}
                                    </span>
                                  </div>
                                ) : (
                                  <span className="text-sm text-gray-400">Nie eingeloggt</span>
                                )}
                              </TableCell>
                                      ...
                                      {employee.isDeleted ? (
                                        ...
                                        <>
                                          ...
                                          <Button
                                            ...
                                            aria-label={`Mitarbeiter "${employee.firstName} ${employee.lastName}" bearbeiten`}
                                            title="Bearbeiten"
                                          >
                                          ...
                                          <Button
                                            ...
                                            aria-label={`Mitarbeiter "${employee.firstName} ${employee.lastName}" löschen`}
                                            title="Löschen"
                                          >
