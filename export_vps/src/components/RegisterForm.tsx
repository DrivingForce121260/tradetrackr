...
interface RegisterFormProps {
  ...
  startWithNewCustomer?: boolean;
}

const RegisterForm: React.FC<RegisterFormProps> = ({ onSuccess, onBack, onShowAGB, onShowDatenschutz, startWithNewCustomer = false }) => {
  const { signUp, signIn } = useAuth();
  const { toast } = useToast();
  const [isLoading, setIsLoading] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  const [registrationStep, setRegistrationStep] = useState<'customer-type' | 'verification-code' | 'form'>(startWithNewCustomer ? 'form' : 'customer-type');
  const [isExistingCustomer, setIsExistingCustomer] = useState(startWithNewCustomer ? false : false);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-blue-800">Verifizierungscode bestätigt</span>
              </div>
