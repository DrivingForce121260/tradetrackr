...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (user && user.concernID && !user.isDemoUser) {
      ...
      // Kurze Verzö¶gerung, um sicherzustellen, dass der Benutzer vollstö¤ndig geladen ist
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        try {
          ...
          // Suche nach Benutzern mit dieser E-Mail, die mö¶glicherweise noch einen Verifizierungscode haben
          const usersRef = collection(db, 'users');
          ...
          if (!querySnapshot.empty) {
            ...
            if (userWithVerificationCode) {
              ...
              throw new Error('Ihre Registrierung ist noch nicht vollstö¤ndig abgeschlossen. Bitte warten Sie einen Moment und versuchen Sie es erneut, oder kontaktieren Sie den Administrator.');
            }
            ...
            if (userWithoutFirebaseUID) {
              ...
              throw new Error('Ihre Registrierung konnte nicht vollstö¤ndig abgeschlossen werden. Bitte kontaktieren Sie den Administrator oder versuchen Sie die Registrierung erneut.');
            }
          }
  ...
  const signUp = async (email: string, password: string, userData: Partial<User>) => {
    try {
      ...
      const concernData = {
        dateCreated: new Date(),
        concernName: `${userData.vorname} ${userData.nachname} GmbH`,
      ...
      // Kurze Verzö¶gerung zwischen Concern- und User-Erstellung
  ...
  const canViewReports = (): boolean => {
    if (!user) return false;
    return true; // Alle Benutzer kö¶nnen Berichte einsehen
  };
  ...
  const canViewOwnProjects = (): boolean => {
    if (!user) return false;
    return true; // Alle Benutzer kö¶nnen ihre eigenen Projekte einsehen
  };

  const canViewOwnReports = (): boolean => {
    if (!user) return false;
    return true; // Alle Benutzer kö¶nnen ihre eigenen Berichte einsehen
  };

  const canViewOwnProjectInfo = (): boolean => {
    if (!user) return false;
    return true; // Alle Benutzer kö¶nnen ihre eigenen Projektinformationen einsehen
  };
  ...
  const canUseMessaging = (): boolean => {
    // Alle authentifizierten Benutzer kö¶nnen das Messaging-System nutzen
  ...
  const startAutoSync = async () => {
    ...
    try {
      // ZUERST: Alle Daten sofort laden (nicht nur auf ö„nderungen warten)
